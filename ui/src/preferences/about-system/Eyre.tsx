...
import { useBossHttpPorts, bossMoonConfigDNS } from '../../state/boss';
...
export const Eyre = () => {
    ...
    return(
      <div className="inner-section space-y-8 mb-4">
        ...
        {httpPorts && Object.keys(httpPorts).length >0 && httpPorts?.https !== undefined &&
        <div className='flex items-center space-x-4'>
            <h3 className='text-md font-bold whitespace-nowrap'>HTTPS Port:</h3>
            <p className="leading-5">{httpPorts?.https}</p>
        </div>
        }
        {httpPorts && Object.keys(httpPorts).length > 0 && httpPorts.http === 80 &&
          <div>
